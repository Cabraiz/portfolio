...
function getWalkBob(agent: HomeDrivePedestrianAgent, elapsedSeconds: number): number {
  const phase =
    agent.animationPhase + elapsedSeconds * Math.max(0.6, agent.speedMps) * 3.2;
...
export function composeHomeDriveThreePedestrianInstanceMatrix(
  ...
  return out.copy(TEMP_OBJECT.matrix);
}
...
export function writeHomeDriveThreePedestrianInstanceMatrices(
  ...
  matrices.length = agents.length;

  agents.forEach((agent, index) => {
    const matrix = matrices[index] ?? new Matrix4();
    composeHomeDriveThreePedestrianInstanceMatrix(
      agent,
      elapsedSeconds,
      detailScale,
      matrix,
